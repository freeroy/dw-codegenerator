...
package com.pmparkchina.${projectName}.${moduleName}.model;
 ...
 * 
 */
@Entity(name="com.pmparkchina.${projectName}.${moduleName}.model.${className}")
...
public class ${className} implements java.io.Serializable {
	...
	<#if table.primaryKey??>
		...
		<#if (table.primaryKey.columns?size>1)>
			...
			public static class ${className}Pk implements java.io.Serializable{
				...
				<#list table.primaryKey.columns as column>
					...
					<#if columnTypeToFieldType(column.type)=="java.util.Date">
						@Temporal(
								<#switch column.type>
									<#case 92>
										TemporalType.TIME
					...
					private ${columnTypeToFieldType(column.type)} ${columnNameToFieldName(column.name)};
		...
		<#else>
			...
			<#if column.autoIncrement>
				...
				@SequenceGenerator(name = "generator", sequenceName = "S_${table.name}")
			</#if>
			private ${columnTypeToFieldType(column.type)} ${columnNameToFieldName(column.name)};
	...
	<#list table.foreignKeys as foreignKey>
		...
		<#if foreignModuleName!=moduleName>
			<#assign foreignPackage="com.pmparkchina."+projectName+"."+foreignModuleName+".model."/>
		</#if>
